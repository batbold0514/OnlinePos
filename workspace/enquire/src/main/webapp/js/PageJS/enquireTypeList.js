...
function initValidator() {
	$('#addForm').bootstrapValidator({
        ...
        e.preventDefault();
        $.post('save-enquireType-ajax', $("#addForm").serialize(), function(result) {
        	if (result.trim() != "") {
				errorMessage();
...
var id ,description,position1;
function pageScript() {
	$("#enquireType th:nth-child(1), #enquireType td:nth-child(1)").hide();
	var context = $("#context").text().trim();
	var a = "http://" + window.location.host;
	var b = "/css/copy_csv_xls_pdf.swf";
	var c = a.concat(context);
	var n = c.concat(b);
	$("#enquireType tbody tr").click(function(e) {
	...
	$("#enquireType tbody tr").dblclick(function(e) {
		position1 = oTable1.fnGetPosition(this);
		id = oTable1.fnGetData(position1)[0];
		description = oTable1.fnGetData(position1)[1];
