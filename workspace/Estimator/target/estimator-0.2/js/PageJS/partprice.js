...
function initValidator() {
	$('#addForm').bootstrapValidator({
        ...
        e.preventDefault();
        $.post('part-price-save', $("#addForm").serialize(), function(result) {
        	if (result.trim() != "") {
				errorMessage();
...
var id1,date,fdate,partName,factory,mark,price,emp,desc
function pageScript() {
	$("#partPricetable th:nth-child(1), #partPricetable td:nth-child(1)").hide();
	$("#partPricetable th:nth-child(10), #partPricetable td:nth-child(10)").hide();
	$("#partPricetable th:nth-child(11), #partPricetable td:nth-child(11)").hide();
	$("#partPricetable th:nth-child(12), #partPricetable td:nth-child(12)").hide();
	var context = $("#context").text().trim();
	var a = "http://" + window.location.host;
	var b = "/css/copy_csv_xls_pdf.swf";
	var c = a.concat(context);
	var n = c.concat(b);
	$("#partPricetable tbody tr").click(function(e) {
		...
		partName = oTable1.fnGetData(position1)[5];
		factory = oTable1.fnGetData(position1)[9];
		mark = oTable1.fnGetData(position1)[10];
		price = oTable1.fnGetData(position1)[6];
		emp = oTable1.fnGetData(position1)[11];
		desc = oTable1.fnGetData(position1)[8];
	...
	$("#partPricetable tbody tr").dblclick(function(e) {
		position1 = oTable1.fnGetPosition(this);
		id1 = oTable1.fnGetData(position1)[0];
		date = oTable1.fnGetData(position1)[1];
		fdate = oTable1.fnGetData(position1)[4];
...
//		e.priventDefault();
		$.ajax({
			url : "search-partprice",
			data : $("#searchPartPriceForm").serialize(),
			success : function(result){
				$("#list-result").html(result);
				pageScript();
			}
