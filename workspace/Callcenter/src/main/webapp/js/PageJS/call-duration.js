...
function initValidator(){
	$('#addform').bootstrapValidator({
        ...
        e.preventDefault();
        $.post('save-callduration', $("#addform").serialize(), function(result) {
        	if (result.trim() != "") {
				errorMessage();
...
var callIndex, duration, position1, id1, oTable;
function pageScript() {
	$("#callDurationTable th:nth-child(1), #callDurationTable td:nth-child(1)").hide();
	var context = $("#context").text().trim();
	var a = "http://" + window.location.host;
	var b = "/css/copy_csv_xls_pdf.swf";
	var c = a.concat(context);
	var n = c.concat(b);
	$("#callDurationTable tbody tr").click(function(e) {
