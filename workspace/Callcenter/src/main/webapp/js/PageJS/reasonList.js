...
function initValidator() {
	...
	$("#description2").keydown(
			function(event) {
				$('#addform').bootstrapValidator('updateMessage',
						"description", 'notEmpty', "Нэр хоосон байна");
			});
}
function pageScript() {
	$("#reasonTable th:nth-child(1), #reasonTable td:nth-child(1)").hide();
	var context = $("#context").text().trim();
	var a = "http://" + window.location.host;
	var b = "/css/copy_csv_xls_pdf.swf";
	var c = a.concat(context);
	var n = c.concat(b);
	$("#reasonTable tbody tr").click(function(e) {
...
function events() {
			...
			.click(
					function(e) {
						...
						if (!(id1 == 0 || id1 == 1)) {
							...
							alert('Өөрчлөх боломжгүй бичлэг');
						}
					});
	$("#btnDelete").click(function() {
		if (!(id1 == 0 || id1 == 1)) {
			...
			alert('Устагах боломжгүй бичлэг');
		}
	});
}
