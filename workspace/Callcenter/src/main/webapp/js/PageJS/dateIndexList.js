...
function initValidator(){
	...
	$('#addform').bootstrapValidator({
        ...
        e.preventDefault();
        $.post('save-dateIndex-ajax', $("#addform").serialize(), function(result) {
        	if (result.trim() != "") {
				errorMessage();
...
var min, max,date_index, position1, id1, oTable;
function pageScript() {
	$("#dateIndexTable th:nth-child(1), #dateIndexTable td:nth-child(1)").hide();
	var context = $("#context").text().trim();
	var a = "http://" + window.location.host;
	var b = "/css/copy_csv_xls_pdf.swf";
	var c = a.concat(context);
	var n = c.concat(b);
	$("#dateIndexTable tbody tr").click(function(e) {
		...
		date_index= oTable1.fnGetData(position1)[3];
	...
	$("#btnShow").click(function(e) {
		...
		if (anSelected.length !== 0) {
			...
			$("input[name='date_index']").val(date_index);
