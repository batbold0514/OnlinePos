package mn.infosystems.callcenter.interceptor;
...
/**
 * TransactionalSessionInterceptor allows the user to apply an Open-Transaction-In-View pattern to Struts2 actions.
 */
        ...
        if (transactionManager != null) {
            ...
            try {
                ...
                if (!status.isCompleted()) {
                    ...
                    // for some reason, the transaction was already completed
                    if (LOG.isEnabledFor(Level.WARN)) {
                        LOG.warn("[doIntercept]: Transaction already marked as completed, not performing rollback indicated by uncatched Excption "
                                + e.getMessage());
                    }
                }
